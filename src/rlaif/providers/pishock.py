...
Auth: per-account ``username`` + ``api_key`` (from
https://pishock.com/#/account), plus a per-device ``sharecode``.
...
class PiShockProvider(Provider):
    ...
    def __init__(
        ...
        *,
        ...
        self.label = label
        self._api: Any = _pishock.PiShockAPI(username=username, api_key=api_key)
        self._shocker: Any = self._api.shocker(
            sharecode=sharecode, log_name=log_name, name=label
        )
    ...
    def from_config(cls, raw: dict[str, str], *, label: str) -> PiShockProvider:
        try:
            ...
            raise ValueError(
                f"pishock provider missing required field: {exc.args[0]}"
            ) from exc

    @classmethod
    def from_components(
        cls, *, api: Any, shocker: Any, label: str
    ) -> PiShockProvider:
